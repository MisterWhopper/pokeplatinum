...
int main(int argc, char **argv)
{
    ...
    enum_seq_t items = common_initenum(
        filename, "Item", .sourcefile = __FILE__, .depfile = depfile_path, .outdir = output_dir, .format = DATAPROC_F_JSON, .enums = enums, .archives = archives, .headers = headers, .textbanks = textbanks, .filter = has_data, );
...
static void proc_item(datafile_t *df, const char *member)
{
    ...
    case ITEM_USE_FUNC_NONE:
        if (item.fieldPocket == POCKET_BALLS) {
            item.dummy = iter_balls++;
        } else {
            proc_params(df, &item);
        }
        break;
    ...
    case ITEM_USE_FUNC_SUPER_ROD:
        item.dummy = iter_rods++;
        break;
    ...
    case ITEM_USE_FUNC_TM_HM: {
        ...
        if (move.type == DATAPROC_T_ERR) {
            break;
        }

        datanode_t move_id = dp_lookup(move, "enum Move");
        if (move_id.type != DATAPROC_T_MAPPED) {
            break;
        }
...
static void proc_params(datafile_t *df, ItemData *item)
{
    ...
    if (params.type == DATAPROC_T_NULL) {
        return;
    }
    ...
    item->partyUseParam = (ItemPartyParam) {
        ...
        .rerollAbility = dp_bool(tryparam(params, "rerollAbility")),
...
static void parse_args(int *pargc, char ***pargv)
{
    ...
    while ((c = getopt(*pargc, *pargv, ":o:M:h")) != -1) {
        ...
        case 'h':
            ...
            break;
        case ':':
            usage("missing argument for '-%c'", optopt);
            break;
        default:
    ...
    *pargv += optind;
    if (*pargc < 1) {
        usage("missing required argument ENUMFILE");
    }
    if (*pargc < 2) {
        usage("missing required argument BASEDIR");
    }
